        ...
        if generate_clicked:
            ...
            for q_num, group in df.groupby("Question Number", sort=False):
                ...
                if q_type in ['single2', 'dynamicgrid2']:
                    if not q_text.startswith("{#rp_css}"): q_text = "{#rp_css}" + q_text
                
                if 'dynamicgrid' in q_type:
                    metadata_script = re.sub(r'columnGrid\$all\$width\s*=\s*[\d\.]+', f'columnGrid$all$width = {calculated_width}', metadata_script)
                elif q_type == 'single2':
                    for prop in ['grid$medium$width', 'grid$large$width', 'grid$extralarge$width']:
                        metadata_script = re.sub(prop.replace('$', r'\$') + r'\s*=\s*[\d\.]+', f'{prop} = {calculated_width}', metadata_script)
